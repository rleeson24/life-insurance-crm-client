...
The **API repo** owns the Bicep platform (`life-insurance-crm-api/infra`). This repo only publishes the Vite SPA into the Static Web App that infra already created.

## Split of responsibility

| Repo | What it does |
|------|----------------|
| `life-insurance-crm-api` | Deploys Bicep (SQL, Key Vault, ACR, Container Apps, Static Web App, OIDC). Deploys the API container image. |
| `life-insurance-crm-client` (this repo) | `npm run build` and uploads `dist` to Azure Static Web Apps. |
